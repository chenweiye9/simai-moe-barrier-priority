...
SIMAI_NS3_SOURCE_DIR="${SCRIPT_DIR:?}"/../../../ns-3-alibabacloud/simulation
SIM_LOG_DIR="${SIMAI_LOG_DIR:-${HOME}/.simai}"
...
export CC="${CC:-gcc-13}"
export CXX="${CXX:-g++-13}"
NS3_MODULES="${NS3_MODULES:-core;network;internet;applications;point-to-point;traffic-control;csma;mtp}"
...
function compile {
    ...
    # fi
    if [ -d "${SIMAI_NS3_SOURCE_DIR}/src" ]; then
        echo "Syncing ns-3 sources from ${SIMAI_NS3_SOURCE_DIR}/src"
        cp -r "${SIMAI_NS3_SOURCE_DIR}/src/." "${NS3_DIR}"/simulation/src/
    fi
    ...
    ./ns3 configure -d debug --enable-mtp --disable-examples --disable-tests --enable-modules "${NS3_MODULES}"
    ./ns3 build
