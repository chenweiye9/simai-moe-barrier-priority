...
OUT_DIR="${ROOT_DIR}/experiments/moe_small/output_1tor"
...
SENDER_COUNT="${SIMAI_INCAST_SENDERS:-7}"
...
run_case() {
  ...
  local enable="$4"
  local max_active="$5"
  local completed_ratio="$6"
  local laggard_ratio="$7"
  local min_remaining="$8"
  ...
  "${SIM_BIN}" -t "${THREADS}" -w "${WORKLOAD}" -n "${topo_file}" -c "${CONFIG}" \
    > "${stdout_file}" 2>&1
...
run_case one_tor_uniform_q8_baseline "${ROOT_DIR}/experiments/moe_small/topo_8g_1gps_1tor_100g.txt" 8 0 1 1.0 1.0 0
run_case one_tor_uniform_q8_laggard "${ROOT_DIR}/experiments/moe_small/topo_8g_1gps_1tor_100g.txt" 8 1 7 0.25 1.10 4194304
run_case one_tor_uniform_q16_baseline "${ROOT_DIR}/experiments/moe_small/topo_8g_1gps_1tor_100g.txt" 16 0 1 1.0 1.0 0
run_case one_tor_uniform_q16_laggard "${ROOT_DIR}/experiments/moe_small/topo_8g_1gps_1tor_100g.txt" 16 1 7 0.25 1.10 4194304
run_case one_tor_straggler_q8_baseline "${ROOT_DIR}/experiments/moe_small/topo_8g_1gps_1tor_100g_straggler.txt" 8 0 1 1.0 1.0 0
run_case one_tor_straggler_q8_laggard "${ROOT_DIR}/experiments/moe_small/topo_8g_1gps_1tor_100g_straggler.txt" 8 1 7 0.25 1.10 4194304
run_case one_tor_straggler_q16_baseline "${ROOT_DIR}/experiments/moe_small/topo_8g_1gps_1tor_100g_straggler.txt" 16 0 1 1.0 1.0 0
run_case one_tor_straggler_q16_laggard "${ROOT_DIR}/experiments/moe_small/topo_8g_1gps_1tor_100g_straggler.txt" 16 1 7 0.25 1.10 4194304

for f in "${OUT_DIR}"/*.summary.txt; do
  echo "== $(basename "${f}")"
  sed -n '1,20p' "${f}"
done
